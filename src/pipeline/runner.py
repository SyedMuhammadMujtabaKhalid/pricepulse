...
class PricePulsePipeline:
    def __init__(
        self, sources: list[BaseSource], notifiers: list[BaseNotifier] | None = None
    ):
    ...
    async def run(self) -> PipelineResult:
        ...
        try:
            ...
            if cleaned:
                ...
                log.warning(
                    "pipeline.store.skipped", reason="No valid records to store"
                )
    ...
    def _store_records(self, records: list[RawPriceRecord]) -> int:
        ...
        with get_session() as session:
            ...
            for i in range(0, len(records), batch_size):
                ...
                if competitors_data:
                    stmt_c = pg_insert(Competitor).values(
                        list(competitors_data.values())
                    )
                ...
                db_comps = (
                    session.query(Competitor.id, Competitor.name)
                    .filter(Competitor.name.in_(comp_names))
                    .all()
                )
                comp_map = {c.name: c.id for c in db_comps}

                db_prods = (
                    session.query(Product.id, Product.sku)
                    .filter(Product.sku.in_(prod_skus))
                    .all()
                )
