        ...
        if not containers:
            ...
            for item in containers:
                try:
                    ...
                    sku_el = await item.query_selector(self.selectors["sku"]) if "sku" in self.selectors else None
                    sku_text = (await sku_el.inner_text()).strip() if sku_el else f"SKU-{abs(hash(name_text)) % 1000000:06d}"

                    # URL
                    url_el = await item.query_selector("a")
                    item_url = (await url_el.get_attribute("href")) if url_el else self.target_url
