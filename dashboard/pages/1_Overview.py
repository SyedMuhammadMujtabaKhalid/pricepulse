...
try:
    ...
    col3.metric("Unacknowledged Alerts", kpis.active_alerts if kpis else 0, 
                delta_color="inverse", delta="Needs Attention" if kpis and kpis.active_alerts > 0 else "All Good")
    ...
    if not alerts_df.empty:
        for _, row in alerts_df.iterrows():
            if row['severity'] == 'critical':
                st.error(f"[{row['created_at'].strftime('%Y-%m-%d %H:%M')}] **{row['message']}**")
            elif row['severity'] == 'high':
                st.warning(f"[{row['created_at'].strftime('%Y-%m-%d %H:%M')}] **{row['message']}**")
            else:
                st.info(f"[{row['created_at'].strftime('%Y-%m-%d %H:%M')}] {row['message']}")
    else:
